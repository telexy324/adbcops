...
- “智能分析”页面新增多轮 RCA 工作台，可直接提交自然语言问题，并在执行前编辑环境、服务和默认 30 分钟时间窗；
- 创建前展示解析后的 Scope 和歧义提示，创建后按 1.5 秒轮询 Run 详情，分别展示 Round 1～3、并行动作、Skill、Evidence、假设变化和继续原因；
- 支持日志、指标、知识、拓扑和 TiDB 慢 SQL 详情；拓扑以节点与方向边展示，慢 SQL 仅展示指纹及脱敏语句；
- `partial_success`、超时、权限不足、缺失证据和普通失败采用不同状态及说明；
- 活跃 Run ID 同步到 URL 和本地存储，刷新后自动恢复；提供历史 Run 切换、取消和“恢复并重试”，成功动作不会在恢复时重复执行；
- 最终报告展示影响范围、证据强度、支持/反证、缺失证据和只读建议，FACT、RULE、KNOWLEDGE、HYPOTHESIS 均有显式标签；
- 页面不提供自动修复、任意 SQL 或写入入口，并提供标准三轮演示 Fixture；
- 新增多轮工作台专项测试并纳入页面回归测试，覆盖创建、轮询、取消、恢复、权限、超时、部分成功、脱敏 SQL、拓扑和刷新恢复。
